...
const loadSavedOrder = () => {
  ...
  try {
    const saved = localStorage.getItem('platform-orders-tech')
    if (saved) {
...
const techPlatforms = ref(loadSavedOrder())
...
// 初始化平台数据状态
techPlatforms.value.forEach(platform => {
...
// 拖拽排序处理
const onDragEnd = () => {
  console.log(`科技资讯平台排序已更新:`, techPlatforms.value.map(p => p.title))
  // 保存当前排序
  saveOrder(techPlatforms.value)
}
      ...
      <div class="flex-1 overflow-hidden bg-muted/20">
        <UiScrollArea class="h-full">
          <div class="p-4">
            ...
            >
              <NewsRankCard
                  v-for="platform in techPlatforms"
