...
const loadSavedOrder = () => {
  ...
  try {
    const saved = localStorage.getItem('platform-orders-social')
    if (saved) {
...
const saveOrder = (platforms: typeof defaultPlatforms) => {
  if (import.meta.client) {
    try {
      ...
      localStorage.setItem('platform-orders-social', JSON.stringify(order))
    } catch (error) {
...
const socialPlatforms = ref(loadSavedOrder())
...
const fetchAllPlatformsData = async () => {
  const promises = socialPlatforms.value.map(p => fetchPlatformData(p.platform))
...
// 拖拽排序处理
const onDragEnd = () => {
  console.log(`社会新闻平台排序已更新:`, socialPlatforms.value.map(p => p.title))
  // 保存当前排序
  saveOrder(socialPlatforms.value)
}
          ...
          <div class="p-4">
            ...
            <div class="flex items-center justify-between mb-6 ml-5">
              <p class="text-sm text-muted-foreground">
                共 {{ socialPlatforms.length }} 个社会新闻平台，支持拖拽排序
              </p>
            </div>

            <!-- 新闻卡片网格 - 响应式网格布局 -->
            <VueDraggable
                v-model="socialPlatforms"
                :animation="200"
                handle=".drag-handle"
